<header>
    <nav class="navbar navbar-expand-lg border-bottom darkLight d-flex justify-content-between">
        <div>
            <div>
                <a class="navbar-brand ps-3" href="{{ route('homepage') }}">
        ...
        <div>
            <form class="d-flex position-relative" role="search" action="{{route('announcement.search')}}" method="GET">
                <input class="form-control me-2 input-search" type="search" name="searched" placeholder="Cosa stai cercando?" aria-label="Search">
                <button class="btn btn-search position-absolute end-0" type="submit">
                    <i class="bi bi-search-heart text-light fs-5"></i>
                </button>
            </form>
        </div>
